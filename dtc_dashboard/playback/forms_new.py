    ...
    def __init__(self, *args,**kwargs):
        ...
        if(len(args) > 0):
            if('oldform' in args[0]):
                ...
                # self.vehicle_state = False
                choices = getvehicles(routeId= oldform['route_id'],
                startTime=appendTimeZone(oldform['stime']),
                endTime=appendTimeZone(oldform['etime']))
                self.fields['vehicle_id_f'] = forms.ChoiceField(label = "Vehicle Ids",
                required=True,widget=Select2Widget,choices=choices)
                self.fields.pop('startDateTime')
                self.fields.pop('endDateTime')
                self.fields.pop('route_id_f')
        # else:
        #     self.fields['startDateTime']= DateTimeField(input_formats=["%Y-%m-%d %H:%M:%S"],widget=forms.TextInput(attrs={'placeholder': 'YYYY-mm-dd HH:MM:SS'}))
        #     self.fields['endDateTime']= DateTimeField(input_formats=["%Y-%m-%d %H:%M:%S"],widget=forms.TextInput(attrs={'placeholder': 'YYYY-mm-dd HH:MM:SS'}))
        #     self.fields['route_id_f'] = forms.ChoiceField(label = "Route Ids",choices=getroutes(),widget=Select2Widget)
